...
void loadWarehouse () {
    for (Integer i = 0; i < warehouseCount; i++) {
        warehouse.insert ({i + 1}, {randomastring<10> (6, 10), randomastring<20> (10, 20),
...
void loadStock (Integer w_id) {
    for (Integer i = 0; i < ITEMS_NO; i++) {
        ...
        stock.insert (
            {w_id, i + 1},
            {randomNumeric (10, 100), randomastring<24> (24, 24), randomastring<24> (24, 24),
             randomastring<24> (24, 24), randomastring<24> (24, 24), randomastring<24> (24, 24),
             randomastring<24> (24, 24), randomastring<24> (24, 24), randomastring<24> (24, 24),
             randomastring<24> (24, 24), randomastring<24> (24, 24), 0, 0, 0, s_data});
    }
}

void loadDistrict (Integer w_id) {
    for (Integer i = 1; i < 11; i++) {
        district.insert ({w_id, i}, {randomastring<10> (6, 10), randomastring<20> (10, 20),
                                     randomastring<20> (10, 20), randomastring<20> (10, 20),
                                     randomastring<2> (2, 2), randomzip (),
...
void loadCustomer (Integer w_id, Integer d_id) {
    ...
    for (Integer i = 0; i < 3000; i++) {
        ...
        customer.insert (
            {w_id, d_id, i + 1},
            {c_first, "OE", c_last, randomastring<20> (10, 20), randomastring<20> (10, 20),
             randomastring<20> (10, 20), randomastring<2> (2, 2), randomzip (),
             randomnstring (16, 16), now, c_credit, 50000.00, randomNumeric (0.0000, 0.5000),
        ...
        history.insert ({t_id, h_id},
                        {i + 1, d_id, w_id, d_id, w_id, now, 10.00, randomastring<24> (12, 24)});
    }
}
    ...
    for (Integer o_c_id : c_ids) {
        ...
        for (Integer ol_number = 1; ol_number <= o_ol_cnt; ol_number++) {
            ...
            orderline.insert (
                ...
                {ol_i_id, w_id, ol_delivery_d, 5, ol_amount, randomastring<24> (24, 24)});
        }
        o_id++;
    }

    for (Integer i = 2100; i <= 3000; i++) neworder.insert ({w_id, d_id, i}, {});
}
...
void newOrder (Integer w_id, Integer d_id, Integer c_id, const vector<Integer>& lineNumbers,
    ...
    for (unsigned i = 0; i < lineNumbers.size (); i++) {
        ...
        orderline.insert ({w_id, d_id, o_id, lineNumber},
...
void paymentById (Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id, Integer c_id,
    ...
    Integer h_id = (Integer)leanstore::WorkerCounters::myCounters ().variable_for_workload++;
    history.insert ({t_id, h_id},
                    {c_id, c_d_id, c_w_id, d_id, w_id, datetime, h_amount, h_new_data});
}
