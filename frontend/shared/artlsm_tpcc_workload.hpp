...
Varchar<16> randomnstring (Integer minLenStr, Integer maxLenStr) {
    Integer len = rnd (maxLenStr - minLenStr + 1) + minLenStr;
    Varchar<16> result;
    for (Integer i = 0; i < len; i++) result.append (48 + rnd (10));
    return result;
}
...
void loadItem () {
    for (Integer i = 1; i <= ITEMS_NO; i++) {
        ...
        item.insert ({i}, {{i},
                           randomId (1, 10000),
                           randomastring<24> (14, 24),
                           randomNumeric (1.00, 100.00),
                           i_data});
    }
}
...
void loadWarehouse () {
    for (Integer i = 0; i < warehouseCount; i++) {
        warehouse.insert ({i + 1}, {{i + 1},
                                    randomastring<10> (6, 10),
                                    randomastring<20> (10, 20),
                                    randomastring<20> (10, 20),
                                    randomastring<20> (10, 20),
                                    randomastring<2> (2, 2),
                                    randomzip (),
                                    randomNumeric (0.1000, 0.2000),
                                    3000000});
    }
}
...
void loadStock (Integer w_id) {
    for (Integer i = 0; i < ITEMS_NO; i++) {
        ...
        stock.insert ({w_id, i + 1}, {{w_id, i + 1},
                                      randomNumeric (10, 100),
...
void loadCustomer (Integer w_id, Integer d_id) {
    ...
    for (Integer i = 0; i < 3000; i++) {
        ...
        customer.insert ({w_id, d_id, i + 1}, {{w_id, d_id, i + 1},
                                               c_first,
                                               "OE",
                                               c_last,
                                               randomastring<20> (10, 20),
        ...
        customerwdl.insert ({w_id, d_id, c_last, c_first}, {{w_id, d_id, c_last, c_first}, i + 1});
        ...
        history.insert (
            {t_id, h_id},
            {{t_id, h_id}, i + 1, d_id, w_id, d_id, w_id, now, 10.00, randomastring<24> (12, 24)});
    }
}
...
void loadOrders (Integer w_id, Integer d_id) {
    ...
    for (Integer i = 1; i <= 3000; i++) c_ids.push_back (i);
    ...
    for (Integer o_c_id : c_ids) {
        ...
        order.insert ({w_id, d_id, o_id},
                      {{w_id, d_id, o_id}, o_c_id, now, o_carrier_id, o_ol_cnt, 1});
        if (FLAGS_order_wdc_index) {
            order_wdc.insert ({w_id, d_id, o_c_id, o_id}, {{w_id, d_id, o_c_id, o_id}});
        }
        ...
        for (Integer ol_number = 1; ol_number <= o_ol_cnt; ol_number++) {
            ...
            orderline.insert ({w_id, d_id, o_id, ol_number}, {{w_id, d_id, o_id, ol_number},
                                                              ol_i_id,
                                                              w_id,
                                                              ol_delivery_d,
                                                              5,
    ...
    for (Integer i = 2100; i <= 3000; i++) neworder.insert ({w_id, d_id, i}, {{w_id, d_id, i}});
}

// run
void newOrder (Integer w_id, Integer d_id, Integer c_id, const vector<Integer>& lineNumbers,
    ...
    for (Integer sw : supwares)
        if (sw != w_id) all_local = 0;
    Numeric cnt = lineNumbers.size ();
    Integer carrier_id = 0; /*null*/
    order.insert ({w_id, d_id, o_id},
                  {{w_id, d_id, o_id}, c_id, timestamp, carrier_id, cnt, all_local});

    if (FLAGS_order_wdc_index) {
        order_wdc.insert ({w_id, d_id, c_id, o_id}, {{w_id, d_id, c_id, o_id}});
    }
    neworder.insert ({w_id, d_id, o_id}, {{w_id, d_id, o_id}});
    ...
    for (unsigned i = 0; i < lineNumbers.size (); i++) {
        ...
        orderline.insert ({w_id, d_id, o_id, lineNumber}, {{w_id, d_id, o_id, lineNumber},
                                                           itemid,
                                                           supware,
                                                           ol_delivery_d,
                                                           qty,
...
void delivery (Integer w_id, Integer carrier_id, Timestamp datetime) {
    for (Integer d_id = 1; d_id <= 10; d_id++) {
        ...
        // order.scan({w_id, d_id, o_id}, [&](const order_t& rec) { ol_cnt = rec.o_ol_cnt; c_id =
        // rec.o_c_id; return false; }); if (ol_cnt == minInteger) continue;
        Integer ol_cnt, c_id;
        ...
        order.scan (
            ...
            [&] () { is_safe_to_continue = false; });
        if (!is_safe_to_continue) continue;
        order.update1 (
            {w_id, d_id, o_id}, [&] (order_t& rec) { rec.o_carrier_id = carrier_id; },
            WALUpdate1 (order_t, o_carrier_id));
        // -------------------------------------------------------------------------------------
        // First check if all orderlines have been inserted, a hack because of the missing
...
void stockLevel (Integer w_id, Integer d_id, Integer threshold) {
    ...
    //"SELECT COUNT(DISTINCT (S_I_ID)) AS STOCK_COUNT FROM orderline, stock WHERE OL_W_ID = ? AND
    // OL_D_ID = ? AND OL_O_ID < ? AND OL_O_ID >= ? AND
...
void orderStatusName (Integer w_id, Integer d_id, Varchar<16> c_last) {
    ...
    if (c_count == 0) return;  // TODO: rollback
    unsigned index = c_count / 2;
    if ((c_count % 2) == 0) index -= 1;
...
void paymentById (Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id, Integer c_id,
    ...
    if (c_credit == "BC") {
        ...
        auto numChars =
            snprintf (c_new_data.data, 500, "| %4d %2d %4d %2d %4d $%7.2f %lu %s%s %s", c_id,
    ...
    history.insert (
        {t_id, h_id},
        {{t_id, h_id}, c_id, c_d_id, c_w_id, d_id, w_id, datetime, h_amount, h_new_data});
}

void paymentByName (Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id, Varchar<16> c_last,
                    Timestamp h_date, Numeric h_amount, Timestamp datetime) {
    Varchar<10> w_name;
    Varchar<20> w_street_1;
    Varchar<20> w_street_2;
...
void paymentRnd (Integer w_id) {
    ...
    // if (urand (1, 100) <= 60) {
    //     paymentByName (w_id, d_id, c_w_id, c_d_id, genName (getNonUniformRandomLastNameForRun
    //     ()),
    //                    h_date, h_amount, currentTimestamp ());
    // } else {
