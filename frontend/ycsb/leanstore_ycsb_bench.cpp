...
using YCSBKey = u64;
using YCSBPayload = BytesPayload<CMAKE_CONFIG_VALUE_SIZE>;
...
public:
    ...
    void DoWrite2 (ThreadState* thread) {
        ...
        while (ops < maximum) {
            ...
            for (; j < batch && ops < maximum; j++, ops++) {
                auto key = ART_OLC_X::Random::next48 ();
                YCSBPayload payload;
                getInt (reinterpret_cast<u8*> (&payload), sizeof (YCSBPayload), key);
    ...
    void DoWriteStr (ThreadState* thread) {
        uint64_t batch = FLAGS_batch;
        if (key_trace_ == nullptr) {
            ERROR ("DoWrite lack key_trace_ initialization.");
            return;
    ...
    void DoReadLatest (ThreadState* thread) {
        ...
        size_t start_offset = thread->tid * interval;
        // Read the latest 20%
        auto key_iterator = key_trace_->read_iterate_between (start_offset + 0.8 * interval,
    ...
    void DoSaveLoad2Trace (ThreadState* thread) {
        ...
        for (size_t i = 0; i < FLAGS_num; i++) {
            key_trace_->keys_[i] = ART_OLC_X::Random::next48 ();
        }
    ...
    void DoStats (ThreadState* thread) {
        ...
        INFO ("Inserted volume: (pages, MiB) = (%lu, %lu)", written_pages, mib);
        auto& table = *adapter;
        table.ToStats ();
    }
